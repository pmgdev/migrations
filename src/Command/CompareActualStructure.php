...
final class CompareActualStructure extends Command\Command
{
	...
	private function checkDiff(array $files): void
	{
		...
		foreach ($files as $file) {
			...
			if ($originalContent === FALSE) {
				throw new \RuntimeException(sprintf('Can not read file %s', $file));
			} else if (@file_put_contents($file, trim(str_replace("\r\n", "\n", (string) preg_replace('~--.*~', '', $originalContent)))) === FALSE) {
				throw new \RuntimeException(sprintf('Can not write to file %s', $file));
			}
			$md5[] = md5_file($file);
		}

		if (count(array_unique($md5)) !== 1) {
			throw new \RuntimeException(sprintf('ERROR - %s and database structure are different, run "diff %s" and update database structure...', self::STRUCTURE, implode(' ', $files)));
		}
	}
	...
	private function updateStructure(string $updateScript): string
	{
		...
		if ($return > 0) {
			throw new \RuntimeException(sprintf('Failed update testing structure with code: %d. Is "tests" directory presents? Output was: %s', $return, $output));
		} else if (preg_match('~(_csfd_test_[a-z0-9]+)~', $output, $matches) !== 1) {
			throw new \RuntimeException(sprintf('Can not detect DB name from output: %s', $output));
		}
		return $matches[0];
	}
